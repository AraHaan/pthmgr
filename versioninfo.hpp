/*
	Defines Version information.
*/

#ifndef PTHMGR_VERSIONINFO_HPP
/* version information included. */
#define PTHMGR_VERSIONINFO_HPP
/* version information. */
#define PTHMGR_MAJOR 0
#define PTHMGR_MINOR 0
#define PTHMGR_RELEASE 3
#define PTHMGR_BUILD 0

/* Main version info things. */
#define PTHMGR_DESCRIPTION "Windows(R) Path Environment Variable Manager"
#define PTHMGR_VERSION "0.0.3"
#define INTERNAL_NAME "pthmgr"
#define LEGAL_COPYRIGHT "Copyright (C) 2017"
#define ORIGINAL_FILENAME "pthmgr.exe"
#endif
